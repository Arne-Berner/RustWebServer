...
A workspace can be used similarly to mods, so that only one cargo.lock and one target will be produced.  
A workspace will specify its members by using a cargo.toml which has no package or dependencies section, but has a [workspace] section instead. The benefit of using libraries instead of modules is that you can swap the libraries and share the libraries between applications. You can also host your libraries on crate.io. But beware, libraries on crate.io cannot be deleted.
...
We will have one root composite binary, which will use actix or tokio to form an async main function. This binary will depend on a controller (that uses actix for incoming requests), a business layer (with our business specific functions) and a datalayer (with a connection to our postgres server via seaORM).
